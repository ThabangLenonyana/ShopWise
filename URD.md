...
| **Backend (API Layer)**           | Provides RESTful APIs for frontend interactions (product listings, price comparisons, recommendations).            | - Node.js (Express) or Python (Flask/Django)                                                                     | - Endpoints: <br>  • `GET /categories` <br>  • `GET /products` <br>  • `GET /product` <br>  • `POST /compare-prices` <br>  • `GET /recommendations`<br>- Security: OAuth 2.0 or JWT for authentication     |
